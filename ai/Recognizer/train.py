...
def train(train_data_dir, test_data_dir, model_dir, log_dir, batch_size, epochs, learning_rate=0.001):
    check(model_dir)
    check(log_dir)
    ...
    info('Adding Custom Layers to VGG16')
    # add custom layers
    x = model.output
...
if __name__ == "__main__":
    ...
    if os.path.exists(train_data_dir) and os.path.exists(test_data_dir):
        ...
        train(train_data_dir, test_data_dir, model_dir, log_dir, batch_size, epochs, learning_rate)
    else:
